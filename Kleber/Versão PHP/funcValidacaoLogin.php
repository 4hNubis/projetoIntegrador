...
if (isset($_POST['btEntrar'])):
	...
	if(empty($email) or empty($senha)):
		...
		$erros[] = "<p> O campo login e/ou senha precisa estar preenchido </p>";
	else:
		//criptografando a senha no mode BCrypt
		$senha=sha1($senha);
		// Faz a busca e comparação via codigo SQL
		$sql= "SELECT idUsuario,nomeUsuario,admin FROM usuario WHERE email= '$email' AND senha='$senha'";
		...
		if (mysqli_num_rows($resultado) > 0):
			...
			//Redireciona para página dekasConfeitaria.php
